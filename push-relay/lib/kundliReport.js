...
async function handleReportStatus(req, res) {
  ...
  // Email the PDF (best-effort, async). Skipped when the order
  // was AUTO-GENERATED by saving a kundli profile (no explicit
  // user click) - per user requirement 2026-05-28 the customer
  // should receive a push notification rather than an unsolicited
  // email. Explicit "Send via email" button clicks still email
  // because skipEmail is only true on the auto-trigger path.
