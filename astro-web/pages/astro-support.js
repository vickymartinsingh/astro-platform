...
const ASTRO_FAQS = [
  ['When do I get paid?',
    'Earnings settle to your wallet after each session. Request a '
    + 'payout from Profile; payouts are processed within 24-48h.'],
  ['A client was abusive / the session had issues.',
    'Raise a ticket under "Customer / Session issue" with the details '
    + 'and our Quality Team will review it.'],
  ['I cannot log in / go online.',
    'Raise a "Login / Account access" ticket. Make sure your profile '
    + 'is approved by admin first.'],
];
...
export default function AstroSupport() {
  ...
  async function submitNew() {
    ...
    try {
      const r = await ticketService.createTicket(user.uid, {
        ...f, name: profile?.name, role: 'astrologer' });
      setF({ category: 'customer', subject: '', message: '' });
      setView(r.id);
    } catch (e) {
        ...
        <div className="surface space-y-3 p-4">
          <label className="block text-sm">
            ...
            <select className="input mt-1" value={f.category}
              ...
              {ASTRO_TICKET_CATEGORIES.map(([k, lbl]) => (
          ...
          <p className="text-xs text-sub-text">
            You will get a ticket number and a notification. Replies
            from the admin team appear right here.
          </p>
        </div>
      </Layout>
    );
  ...
  return (
    <Layout>
      ...
      <p className="mb-3 text-sm text-sub-text">
        Raise a ticket for payouts, login, customer, technical or any
        other issue. Our team replies here.
      </p>
