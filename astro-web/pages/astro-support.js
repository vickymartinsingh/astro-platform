...
export default function AstroSupport() {
  ...
  useEffect(() => {
    ...
    supportService.ensureTicket(user.uid,
      { name: profile?.name, role: 'astrologer' }).catch(() => {});
  ...
  async function send() {
    ...
    await supportService.sendSupport(user.uid, user.uid, v,
      { name: profile?.name, role: 'astrologer' });
  }
  ...
  return (
    <Layout>
      ...
      <p className="mb-3 text-sm text-sub-text">
        Reach the AstroConnect team for payouts, account or technical
        help. Replies appear here.
      </p>
      <div className="surface flex h-[65vh] flex-col p-0">
        <div className="flex-1 space-y-2 overflow-y-auto p-4">
          {msgs.length === 0 && (
            <div className="text-sm text-sub-text">
              Send your query and the team will reply here.
            </div>
          )}
          {msgs.map((m) => {
