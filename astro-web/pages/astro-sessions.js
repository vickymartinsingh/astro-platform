...
export default function AstroSessions() {
  ...
  useEffect(() => {
    if (!user) return;
    sessionService.getAstrologerSessions(user.uid).then(async (list) => {
  ...
  return (
    <Layout>
      <h1 className="mb-3 text-xl font-bold">My Sessions</h1>
        ...
        <table className="w-full text-sm">
          <thead className="text-left text-sub-text">
            <tr>
              ...
              <th className="p-2">Net</th><th className="p-2">Status</th>
            </tr>
          </thead>
          <tbody>
            {shown.map((s) => (
              <tr key={s.id} className="border-t">
                ...
                <td className="p-2">₹{s.astrologerEarning || 0}</td>
