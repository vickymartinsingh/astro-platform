...
// Android hardware BACK (admin app). Single press never closes the app:
// go to the previous screen, or on a root screen require a quick
// double-press then a confirm dialog before exiting.
const ROOTS = ['/', '/admin-dashboard', '/admin-login'];
...
export default function useNativeBack() {
  ...
  useEffect(() => {
    ...
    const sub = App.addListener('backButton', () => {
      ...
      if (!onRoot) { router.replace('/admin-dashboard'); return; }
      const now = Date.now();
