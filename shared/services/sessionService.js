...
async function newSessionId() {
  ...
  // we never block the customer. Still numeric; just longer.
...
export function sessionRefNo(idOrSession) {
  ...
  if (!id) return '';
  // NEW sessions use 8-digit numeric ids - show them in full (those
  // ARE the customer-facing ref). LEGACY Firestore-autogenerated
  // UUIDs are sliced to the last 6 chars + uppercased, the old
  // display behaviour, so historical receipts stay recognisable.
