...
function ApiPdfHero({ kundli }) {
      ...
      try {
        ...
        unsub = onSnapshot(q, (snap) => {
          ...
          if (gen) {
            ...
            // AUTO-RECOVERY for stuck-generating orders:
            //   Tier 1 (>2 min): rescue probe. Cheap - just asks the
            //     relay whether the PDF is cached on R2 / AstroSeer.
            //     No Firestore write, no AstroSeer regen, no cost.
            //   Tier 2 (>5 min AND rescue empty): ONE retry via
            //     requestReport. Covers orders that were created but
            //     never actually processed (Render dyno was asleep
            //     when webhook fired, AstroSeer was down, etc.) The
            //     5-min cooloff + the per-mount _retriedKundliOrders
            //     set guarantee we fire at most one retry per order
            //     per browser session - quota safe.
            ...
            if (ageMs > 2 * 60 * 1000) {
              (async () => {
                ...
                // Tier 2: rescue had nothing AND order is genuinely
                // stale. Fire ONE retry then let the existing
                // listener pick up the new status.
                if (ageMs > 5 * 60 * 1000 && kundli && kundli.id
                  && kundli.userId) {
                  try {
                    if (typeof window !== 'undefined') {
                      window.__retriedKundliOrders =
                        window.__retriedKundliOrders || new Set();
                    }
                    const seen = (typeof window !== 'undefined'
                      && window.__retriedKundliOrders) || null;
                    if (seen && !seen.has(gen.id)) {
                      seen.add(gen.id);
  ...
  function open() {
    ...
    // STILL GENERATING: the background poller (every 25 s) is already
    // hammering rescuePdfByOrderId for us. The page-load effect ALSO
    // fires a one-shot rescue if the order is older than 2 minutes.
    // View click MUST stay PASSIVE here - the previous version called
    // requestReport on every click which burned AstroSeer API quota
    // for nothing (the order was already in flight). We just try ONE
    // cheap rescue probe and otherwise tell the customer to wait.
    if (autoGen.status === 'generating') {
      setProgress('Checking your kundli now...');
      (async () => {
        ...
        setProgress('Your kundli is being prepared. It will open '
          + 'automatically the moment it is ready - usually within a '
          + 'minute. No need to click again.');
  ...
  function downloadNow() {
    ...
    if (autoGen.status === 'generating') {
      // PASSIVE: try one cheap rescue probe, otherwise just inform
      // the customer. The background poller already retries every
      // 25 s - we MUST NOT fire a second requestReport on every
      // click or AstroSeer's API quota burns for nothing.
      setProgress('Checking your kundli now...');
      (async () => {
        if (autoGen.order && autoGen.order.id) {
          const rescued = await kundliService.rescuePdfByOrderId(
        ...
        setProgress('Your kundli is being prepared. The PDF will '
          + 'download automatically when ready - no need to click again.');
        setTimeout(() => setProgress(''), 6000);
      })();
      return;
    }
