    ...
    try {
      ...
      try {
        ...
        if (just) {
          ...
          // the profile is saved. No popup, no email - just kick
          // off the relay so by the time the customer taps the
          // Free Report tab the PDF is either ready or seconds away.
          // autoGenerated:true tags the order so the relay knows
          // to skip the customer email (push notification only).
          // Per user requirement 2026-05-28: this applies to FREE
          // reports only; paid reports stay click-to-buy.
          try {
...
function ApiPdfHero({ kundli }) {
  ...
  // When the autoGen onSnapshot says we already have a ready
  // PDF for this profile, View PDF can short-circuit straight
  // to the viewer. When a generation is in flight, we surface
  // the "Kundli is being generated. We'll notify you shortly..."
  // popup instead of placing a duplicate order.
  const [inFlightPopup, setInFlightPopup] = useState(false);
  function open() {
    if (!kundli || !kundli.id || !kundli.userId) {
      setErr('Save a kundli profile first.'); return;
    }
    setErr('');
    // Fast paths driven by the live Firestore listener:
  ...
  return (
    <div className="mt-3 rounded-card bg-white p-4 shadow-sm">
      ...
      {inFlightPopup && (
        <KundliBeingGeneratedPopup
          orderId={autoGen.order ? autoGen.order.id : null}
          onClose={() => setInFlightPopup(false)} />
      )}
    </div>
  );
}
