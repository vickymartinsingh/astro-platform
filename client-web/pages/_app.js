...
function WithProviders({ children }) {
  ...
  // explicitly dismisses it. This guarantees the customer never sees
  // a white flash between the system launch image and the first React
  // paint - even on a slow cold start. Web/desktop is a no-op because
  // the @capacitor/splash-screen plugin only exists in the native bundle.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
